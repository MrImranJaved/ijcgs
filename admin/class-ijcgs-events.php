...
class IJcgs_Events
{
     ...
     * For Testing purpose 
     */
    public function ijcgs_event_notices()
    {
        // echo "<pre>";
        // echo "</pre>";
    }
     ...
     *  @since     1.0.0
     */
    public function ijcgs_wordpress_newUser( $user_id )
    {
        # if There is a integration on  new user
        if ( $this->ijcgs_integrations( 'wordpress_newUser' )[0] ) {
            ...
            if ( function_exists( 'get_userdata' ) and function_exists( 'get_user_meta' ) ) {
                ...
                $this->common->ijcgs_log(
                    ...
                    __METHOD__,
                    "702",
                    "ijcgs_log: get_userdata or get_user_meta is not Exist"
                );
            }
     ...
     *  @since     1.0.0
     */
    public function ijcgs_wordpress_profileUpdate( $user_id, $old_user_data )
    {
        # if There is a integration on User profile update
        if ( $this->ijcgs_integrations( 'wordpress_UserProfileUpdate' )[0] ) {
            # if get_userdata() and get_user_meta() Functions are exist
     ...
     *  @since    1.0.0
     */
    public function ijcgs_wordpress_deleteUser( $user_id )
    {
        # if There is a integration on Delete user
        if ( $this->ijcgs_integrations( 'wordpress_deleteUser' )[0] ) {
            # if get_userdata() and get_user_meta() Functions are exist
     ...
     * @since     1.0.0
     */
    public function ijcgs_wordpress_userLogin( $username, $user )
    {
        # if There is a integration on user login
        if ( $this->ijcgs_integrations( 'wordpress_userLogin' )[0] ) {
     ...
     * @since   1.0.0
     */
    public function ijcgs_wordpress_userLogout( $userInfo )
    {
        # if There is a integration on user logout
        if ( $this->ijcgs_integrations( 'wordpress_userLogout' )[0] ) {
     ...
     */
    public function ijcgs_wordpress_post( $post_id, $post, $update )
    {
     ...
     */
    public function ijcgs_wordpress_comment( $commentID, $commentApprovedStatus, $commentData )
    {
        # if There is a integration on  Comment
        
        if ( $this->ijcgs_integrations( 'wordpress_comment' )[0] ) {
     ...
     * @param     int     $commentData     	  		Product Post 
     */
    public function ijcgs_wordpress_edit_comment( $commentID, $commentData )
    {
        # if There is a integration on edit Comment
        
        if ( $this->ijcgs_integrations( 'wordpress_edit_comment' )[0] ) {
            # Check Comment ID is exist
            if ( empty($commentID) ) {
                $this->common->ijcgs_log(
                    get_class( $this ),
     ...
     */
    public function ijcgs_woocommerce_product( $new_status, $old_status, $post )
    {
     ...
     */
    public function ijcgs_woocommerce_order_status_changed( $order_id, $this_status_transition_from, $this_status_transition_to )
    {
        # check to see is there any integration on this order change Status.
        if ( !$this->ijcgs_integrations( 'wc-' . $this_status_transition_to )[0] ) {
            return;
        }
        # getting order data
     ...
     * @param     int     $order_id     Order ID
     */
    public function ijcgs_woocommerce_new_order_admin( $order_id )
    {
        $order_data = array();
        ...
        # check to see is there any integration on this order change Status.
        if ( !$this->ijcgs_integrations( "wc-" . $order->get_status() )[0] ) {
            return;
        }
        #
        $order_data['orderID'] = ( method_exists( $order, 'get_id' ) && is_int( $order->get_id() ) ? $order->get_id() : "" );
     ...
     * @param     int     $order_id     Order ID
     */
    public function ijcgs_woocommerce_new_order_checkout( $order_id )
    {
        $order_data = array();
        $order = wc_get_order( $order_id );
        ...
        # check to see is there any integration on this order change Status.
        if ( !$this->ijcgs_integrations( 'wc-new_order' )[0] ) {
            return;
        }
        #
        $order_data['orderID'] = ( method_exists( $order, 'get_id' ) && is_int( $order->get_id() ) ? $order->get_id() : "" );
     ...
     */
    public function ijcgs_cf7_submission( $contact_form )
    {
        $id = $contact_form->id();
        $submission = WPCF7_Submission::get_instance();
        $posted_data = $submission->get_posted_data();
        # if There is a integration on this Form Submission
        if ( !empty($id) and $this->ijcgs_integrations( 'cf7_' . $id )[0] ) {
     ...
     * @param    array     $form_data     data_array
     */
    public function ijcgs_ninja_forms_after_submission( $form_data )
    {
        # if There is a integration on this Form Submission
        
        if ( isset( $form_data["form_id"] ) and $this->ijcgs_integrations( 'ninja_' . $form_data["form_id"] ) ) {
            # Empty array holder
            $data = array();
            # Looping the Fields
     ...
     */
    public function ijcgs_formidable_after_save( $entry_id, $form_id )
    {
        # if There is a integration on this Form Submission
        if ( !empty($form_id) and $this->ijcgs_integrations( 'frm_' . $form_id )[0] ) {
            # Check to see database table exist or not
            
            if ( $this->ijcgs_dbTableExists( "frm_item_metas" ) ) {
                # Code
                $dataArray = array();
                global  $wpdb ;
     ...
     * @param      array    $form_data     	data_array
     */
    public function ijcgs_wpforms_process( $fields, $entry, $form_data )
    {
        # if There is a integration on this Form Submission
        if ( isset( $form_data["id"] ) and $this->ijcgs_integrations( 'wpforms_' . $form_data["id"] )[0] ) {
        }
    }
     ...
     * @since    2.0.0
     */
    public function ijcgs_weforms_entry_submission(
        $entry_id,
        $form_id,
        $page_id,
        $form_settings
    ...
    {
        # if There is a integration on this Form Submission
        if ( !empty($form_id) and $this->ijcgs_integrations( 'we_' . $form_id ) ) {
     ...
     * @since    3.3.0
     */
    public function ijcgs_gravityForms_after_submission( $entry, $formObj )
    {
        # if There is a integration on this Form Submission
        if ( isset( $entry['form_id'] ) and $this->ijcgs_integrations( 'gravity_' . $entry['form_id'] )[0] ) {
        }
    }
     ...
     * @since    3.6.0
     */
    public function ijcgs_forminator_custom_form_submit_field_data( $field_data_array, $form_id )
    {
        # Testing Starts
        # if There is a integration on this Form Submission
        if ( isset( $form_id ) and $this->ijcgs_integrations( 'forminator_' . $form_id )[0] ) {
        }
    }
     ...
     * @since    3.6.0
     */
    public function ijcgs_fluentform_before_submission_confirmation( $entryId, $formData, $form )
    {
        # Testing Starts
        # if There is a integration on this Form Submission
        if ( isset( $form->id ) and $this->ijcgs_integrations( 'fluent_' . $form->id )[0] ) {
        }
    }
     ...
     */
    public function ijcgs_eventBoss(
        $data_source = '',
        $event_name = '',
        $data_array = '',
        $id = ''
    )
    {
        ...
        foreach ( $data_array as $key => $value ) {
            ...
            if ( is_array( $value ) or is_object( $value ) ) {
                ...
                if ( !empty($value) and (is_array( $value ) or is_object( $value )) ) {
                    # Loop the array
                    $size = count( $value );
                    $i = 0;
                    foreach ( $value as $arrKey => $arrValue ) {
                        ...
                        if ( !is_array( $arrValue ) or !is_object( $arrValue ) ) {
                            # removing last comma ( ,)
                            
                            if ( $size == $i ) {
                                $separatedString .= $arrValue;
                            } else {
                            ...
                            $separatedString .= json_encode( $arrValue ) . " ";
                        }
                    
                    }
                } else {
                    # For Object type;
                    $separatedString = json_encode( $value );
                }
                
                # inserting space separated string to the Holder;
                $data_array[$key] = strip_tags( $separatedString );
            } else {
     ...
     */
    public function ijcgs_integrations( $DataSourceID = '' )
    {
        # getting the Options
        $integrations = get_transient( "ijcgs_integrations" );
        ...
        if ( $integrations and is_array( $integrations ) ) {
            ...
            set_transient( 'ijcgs_integrations', $integrationsArray );
            # return  array with First Value as Bool and second one is integrationsArray array
            return array(
                $integrationForDataSource,
                $integrationsArray,
